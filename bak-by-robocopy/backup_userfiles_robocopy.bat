...
REM 輸出來源與目標大小，單位對齊，顯示至小數點一位
powershell -Command ^
  "$src=%SRC_SIZE%; $dst=%DST_FREE%; ^
   $srcGB=[math]::Round($src/1GB,1); ^
   $dstGB=[math]::Round($dst/1GB,1); ^
   Write-Host ('來源資料夾大小：{0,12:N1} GB' -f $srcGB); ^
   Write-Host ('目標磁碟剩餘空間：{0,12:N1} GB' -f $dstGB)"
